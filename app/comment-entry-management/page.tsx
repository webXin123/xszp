...
import { House, NotebookPen, ShieldAlert } from "lucide-react"
...
function CommentEntryManagementPageContent() {
  const { grades, classes, currentTeacher } = useEvaluation()
      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-[#d7def8] bg-white/92 backdrop-blur-xl">
        <div className="mx-auto flex h-14 w-full max-w-[1240px] items-center justify-between gap-4 px-4">
          ...
          <nav className="flex min-w-0 items-center gap-1 text-sm">
            ...
            <span className="flex items-center gap-1.5 rounded-lg bg-primary/10 px-3 py-2 font-semibold text-primary"><NotebookPen className="size-4" aria-hidden="true" />学期评语管理</span>
          </nav>
      ...
      <main className="mx-auto flex w-full max-w-[1240px] flex-col gap-4 pt-1">
        {isDirector ? (
          ...
          <section className="flex min-h-[360px] flex-col items-center justify-center rounded-[26px] border border-[#cbd5f5] bg-white p-8 text-center shadow-[0_24px_52px_-34px_rgba(48,62,139,0.76)]">
            ...
            <h1 className="mt-4 text-lg font-bold text-foreground">暂无学期评语管理权限</h1>
            <p className="mt-1 max-w-md text-sm text-muted-foreground">仅管理员可发布学期评语录入任务并查看教师录入进度。</p>
