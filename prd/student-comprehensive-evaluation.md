...
| --- | --- | --- | --- |
| FR-001 | 根据当前身份和授权范围渲染工作台、菜单、数据与可写操作；除家长端外，角色首页在 PC 端使用固定左侧导航、移动端使用固定底部导航，不显示分组标题；每个角色常用导航（不含数据大屏）最多 5 项，移动端导航自动均分且不横向滚动。首页导航切换仅更新核心 iframe 的链接与主体内容，不跳转或重建首页框架；角色工作台首页快捷入口切换主体时同步激活对应导航 tab；切换角色时 iframe 需携带新身份并刷新，以重载对应工作台数据与权限。首页以外的业务入口进入无左侧与顶部导航的独立页面；班主任不显示成绩上传入口，移动端不显示数据大屏入口，无权限入口不可执行。 | `/`、全局导航、业务独立页 | BR-001、BR-010 | AC-001 |
...
| FR-011 | 支持商品、库存、开放期、兑换条件、购物车、结算、核销和订单导出；管理端商品中心支持按商品分类切换查看。 | `/points-mall*`、`/mall-management` | BR-009 | AC-011 |
...
| NFR-002 | 普通页面适配 375px、768px、1024px、1440px；除家长端外的 PC 角色首页左侧导航保持 240px 固定宽度，主体随剩余视口宽度自适配且不设置最大宽度；移动端角色首页导航改为固定底部导航并为主体预留安全区，导航项自动均分且不横向滚动，不显示数据大屏入口；首页 iframe 填满顶栏与底部导航之间的固定一屏主体区域，外层不滚动，内容溢出仅由 iframe 页面滚动。班级评价页面工作区按内容自然增长，不设置固定高度，内容超出视口时由页面滚动。业务独立页不展示左侧和顶部导航，统一使用主题渐变背景与 16px/24px/28px 响应式内边距。表格和 Excel 预览可在自身容器横向滚动，主体不产生无意义横向滚动。 |
| NFR-003 | 学校和学生数据大屏以 1920×1080 设计画布居中等比缩放，核心信息保持一屏可读。 |
| NFR-004 | 重要控件支持键盘操作、可见焦点和语义标签；状态不只依赖颜色，图表提供文本说明。 |
| NFR-005 | 普通业务页面首次进入和路由切换显示与内容结构一致的骨架；学校/学生数据大屏显示深色全屏骨架，所有骨架均不以居中 Logo 遮挡主体结构。 |
...
| NFR-011 | 移动端评价、发卡和学生端底部导航需避让安全区；固定底部操作栏不遮挡可交互内容，所有主操作保持可见焦点状态，桌面端不改变原有双栏工作流。 |

## 9. 验收标准

| ID | Given / When / Then | 对应需求 |
| --- | --- | --- |
| AC-001 | Given 用户切换为任一角色，when 进入首页或导航，then 仅看到角色允许的入口和数据范围；除家长端外，PC 端首页入口以固定左侧导航连续展示且常用入口不超过 5 项（数据大屏除外），移动端以不横向滚动的均分底部导航展示且隐藏数据大屏入口，班主任不显示成绩上传入口；点击首页导航或角色工作台首页快捷入口后，仅更新核心 iframe 的链接与主体内容，并同步激活对应的侧栏/底部导航 tab，导航与顶栏保持不变，iframe 在固定一屏主体区域内承载页面滚动；when 用户切换角色，then iframe 链接同步携带新的用户标识并重新加载相应角色工作台；家长不能进入教师写入操作。 | FR-001 |
| AC-002 | Given 授权教师进入班级评价，when 为班级或学生提交加分/扣分，then 生成可按日期、对象和类型筛选的评价记录；when 班级/学生列表或评价指标内容超出首屏，then 页面按自然内容高度滚动，不出现固定工作区截断或底部空白。 | FR-002 |
...
| AC-011 | Given 家长兑换商品，when 商城未开放、库存不足、条件不符或积分不足，then 兑换被阻止；when 成功兑换，then 产生待线下核销记录，管理端可筛选并导出当前结果；when 管理员切换商品分类，then 商品中心仅展示对应分类及同步数量。 | FR-011 |
| AC-012 | Given 用户访问任一核心业务页面，then 至少可观察两种不同业务状态；when 完成可写操作并刷新，then 本地结果保留；when 清理站点数据，then 恢复确定性种子。 | FR-012 |
| AC-013 | Given 用户进入或切换任一页面，then 先显示与该路由首屏结构对应的骨架，包含相同的顶栏、侧栏或内容卡片比例；when 进入任一数据大屏，then 显示其深色全屏数据布局骨架，且不以居中 Logo 遮挡主体结构。 | FR-013、NFR-005、NFR-006 |
...
| AC-018 | Given 用户在移动端进入班级评价或奖卡发放，when 选择学生/班级后点击底部下一步，then 进入执行页且顶部提供返回选择；when 未选择对象，then 下一步不可用；Given 学生端移动端，then 底部固定展示首页、荣誉记录、活动列表、积分商城四项 Tab，首页承载其余成长信息；when 任一核心页面加载，then PC 与移动端骨架分别匹配真实页面卡片、工作流和底部导航结构。 | FR-001、FR-002、FR-004、FR-009、FR-013、NFR-011 |
