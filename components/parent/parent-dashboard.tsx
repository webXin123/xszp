...
import { MobileSemesterReport, MobileSemesterScores } from "./mobile-student-semester-content"
...
type MobileHomeContentTab = RecordTab | "semester-scores" | "semester-report"
export type ParentMobileTab = "home" | "honors" | "activities" | "semester-scores" | "semester-report"

const PARENT_DASHBOARD_REFERENCE_DATE = new Date("2026-09-21T04:00:00.000Z")
...
export function ParentDashboard({ mobileTab = "home" }: { mobileTab?: ParentMobileTab }) {
  ...
  const [mobileHomeContentTab, setMobileHomeContentTab] = useState<MobileHomeContentTab>("awards")
  const [activityDialogTarget, setActivityDialogTarget] = useState<Activity | null>(null)

  useEffect(() => {
    if (mobileTab === "honors") {
      setRecordTab("honors")
      setMobileHomeContentTab("honors")
    }
    if (mobileTab === "activities") {
      setRecordTab("activities")
      setMobileHomeContentTab("activities")
    }
    if (mobileTab === "semester-scores") setMobileHomeContentTab("semester-scores")
    if (mobileTab === "semester-report") setMobileHomeContentTab("semester-report")
    if (mobileTab === "home") {
      setRecordTab("awards")
      setMobileHomeContentTab("awards")
    }
  }, [mobileTab])
  ...
  const semester = useMemo(() => getSemesterRange(PARENT_DASHBOARD_REFERENCE_DATE), [])
  const semesterOptions = useMemo(() => getStudentSemesterOptions(PARENT_DASHBOARD_REFERENCE_DATE), [])
  ...
  return (
    <div className={cn("relative flex flex-col gap-3 pb-6 lg:gap-3", styles.parentHome)}>
      ...
      <section className={cn("rounded-[18px] border border-[#d7def8] bg-white p-2 shadow-[0_18px_38px_-34px_rgba(64,80,166,0.68)] sm:p-2.5", mobileTab !== "home" && "hidden lg:block")}>
        <div className="grid gap-2 lg:grid-cols-[minmax(0,1.08fr)_minmax(250px,.72fr)_minmax(250px,.72fr)]">
          <div className="grid gap-2">
            ...
            <section aria-label="积分情况" className="rounded-2xl bg-[#fbfcff] p-2 sm:p-2.5"><div className={styles.parentMetricGrid}><Metric label="学期累计积分" value={semesterEarned} tone="blue" /><Metric label="兑换消耗积分" value={spent} tone="orange" /><Metric label="可用积分" value={balance} tone="green" /><Metric label="累计积分" value={totalEarned} tone="purple" /></div></section>
          </div>
      ...
      <section className={cn("flex min-h-[340px] flex-col rounded-[18px] border border-[#dce3f8] bg-white p-3.5 shadow-[0_18px_40px_-32px_rgba(64,80,166,.65)] sm:p-4", mobileTab !== "home" && "lg:flex")}>
        <div role="tablist" aria-label="本学期成长记录分类" className="hidden w-fit max-w-full gap-1 overflow-x-auto overscroll-x-contain rounded-full border border-[#d5ddf8] bg-white p-1 shadow-[0_8px_18px_-14px_rgba(64,80,166,.75)] [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:inline-flex">
...
function Metric({ label, value, tone }: { label: string; value: number; tone: "blue" | "orange" | "green" | "purple" }) {
  ...
  return <div className={cn("min-w-0 rounded-xl border border-[#e4e8f7] bg-white px-2.5 py-1.5", styles.parentMetricCard)}><p className="truncate text-[11px] font-medium text-muted-foreground">{label}</p><p className={cn("mt-0.5 text-lg font-bold tabular-nums", classes[tone])}>{value}<span className="ml-0.5 text-xs font-semibold">分</span></p></div>
}
