  ...
  return <StandalonePageShell mainId={`points-mall-${kind}`} activeLabel="积分商城" activeIcon="shopping">
    <main id={`points-mall-${kind}`} tabIndex={-1} className="relative flex w-full min-w-0 flex-col gap-3 pb-4 pt-0">
      <header className="overflow-hidden rounded-2xl border border-[#d5e1f6] bg-white shadow-[0_18px_38px_-30px_rgba(48,78,140,0.58)]">
        ...
        <div className="bg-gradient-to-r from-primary/[0.09] via-white to-brand-green/[0.08] px-4 py-2.5 sm:px-5">
          <div className="flex flex-wrap items-center justify-between gap-2.5">
            <div className="flex min-w-0 items-center gap-2.5">
              <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><ShoppingBag className="size-4" aria-hidden="true" /></span>
              <div className="min-w-0"><p className="flex items-center gap-1.5 text-[11px] font-semibold text-primary"><Sparkles className="size-3.5" aria-hidden="true" />成长兑换站</p><h1 className="truncate text-lg font-bold tracking-tight text-foreground">{title}</h1></div>
              <div className="flex min-w-0 max-w-[150px] border-l border-primary/15 pl-2.5 sm:max-w-[190px] sm:pl-3"><span className="block min-w-0 truncate text-sm font-semibold text-foreground">{child?.name ?? "未选择学生"}<span className="hidden font-normal text-xs text-muted-foreground sm:inline"> · {child?.className ?? "请使用家长身份访问商城"}</span></span></div>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="inline-flex min-h-9 items-center gap-1.5 rounded-xl bg-brand-green/10 px-2.5 text-sm text-brand-green"><Coins className="size-4" aria-hidden="true" /><span className="font-bold tabular-nums">{Math.max(0, balance)}</span><span className="text-xs font-semibold">可用积分</span></span>
              <Link href={`/points-mall/records${childQuery}`} className="inline-flex min-h-9 items-center gap-1.5 rounded-xl px-2.5 text-xs font-semibold text-primary transition-colors hover:bg-primary/8 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/45"><ReceiptText className="size-4" aria-hidden="true" /><span className="hidden sm:inline">兑换记录</span></Link>
            </div>
          </div>
        ...
        {homePanel && <div className="border-t border-[#e4ebf7] px-4 py-3 sm:px-5 sm:py-3.5">{homePanel}</div>}
      </header>
      {children}
