...
function MallFrame({ children, kind, title, child, balance, cartCount, homePanel }: {
  children: ReactNode
  kind: MallPageKind
  title: string
  ...
  return <StandalonePageShell mainId={`points-mall-${kind}`} activeLabel="积分商城" activeIcon="shopping">
    <main id={`points-mall-${kind}`} tabIndex={-1} className="relative mx-auto flex w-full max-w-6xl flex-col gap-4 pb-4 pt-1">
      <header className="overflow-hidden rounded-2xl border border-[#d5e1f6] bg-white shadow-[0_18px_38px_-30px_rgba(48,78,140,0.58)]">
        <div className="flex flex-wrap items-center justify-between gap-3 bg-gradient-to-r from-primary/[0.09] via-white to-brand-green/[0.08] px-4 py-3 sm:px-5">
          <div className="flex min-w-0 items-center gap-3">
            <button type="button" aria-label="返回上一级页面" onClick={() => window.history.length > 1 ? router.back() : router.push("/")} className="inline-flex size-10 shrink-0 items-center justify-center rounded-xl border border-primary/15 bg-white text-primary transition-colors hover:bg-primary/8 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/45"><ArrowLeft className="size-4" aria-hidden="true" /></button>
            <div className="min-w-0"><p className="flex items-center gap-1.5 text-xs font-semibold text-primary"><Sparkles className="size-3.5" aria-hidden="true" />成长兑换站</p><h1 className="truncate text-lg font-bold tracking-tight text-foreground">{title}</h1></div>
            <div className="flex min-w-0 items-center gap-2 border-l border-primary/15 pl-3 sm:pl-4"><span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><ShoppingBag className="size-4" aria-hidden="true" /></span><span className="min-w-0"><span className="block truncate text-sm font-semibold text-foreground">{child?.name ?? "未选择学生"}</span><span className="block truncate text-xs text-muted-foreground">{child?.className ?? "请使用家长身份访问商城"}</span></span></div>
          </div>
          <div className="flex items-center gap-2">
            <span className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-brand-green/10 px-3 text-sm text-brand-green"><Coins className="size-4" aria-hidden="true" /><span className="font-bold tabular-nums">{Math.max(0, balance)}</span><span className="text-xs font-semibold">可用积分</span></span>
            <Link href={`/points-mall/records${childQuery}`} className="inline-flex min-h-10 items-center gap-1.5 rounded-xl px-3 text-xs font-semibold text-primary transition-colors hover:bg-primary/8 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/45"><ReceiptText className="size-4" aria-hidden="true" /><span className="hidden sm:inline">兑换记录</span></Link>
          </div>
        </div>
        {homePanel && <div className="border-t border-[#e4ebf7] px-4 py-4 sm:px-5 sm:py-5">{homePanel}</div>}
      </header>
      {children}
...
function MallAccessDenied() {
  return <StandalonePageShell mainId="points-mall-access" activeLabel="积分商城" activeIcon="shopping"><main id="points-mall-access" className="mx-auto flex min-h-[420px] w-full max-w-xl flex-col items-center justify-center rounded-2xl border border-dashed border-primary/25 bg-white/75 p-8 text-center"><ShoppingBag className="size-10 text-primary" aria-hidden="true" /><h1 className="mt-4 text-lg font-bold text-foreground">请使用家长身份访问积分商城</h1><p className="mt-2 text-sm text-muted-foreground">可在右上角身份切换中选择家长身份后，为孩子兑换成长礼物。</p><Link href="/" className="mt-5 inline-flex min-h-10 items-center rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground">返回首页</Link></main></StandalonePageShell>
}
...
export function PointsMallHome() {
  ...
  return <MallFrame kind="home" title="积分商城" child={child} balance={balance} cartCount={cartCount} homePanel={<><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold text-primary">{isOpen ? "正在开放" : "暂未开放"}</p><h2 className="mt-0.5 text-base font-bold text-foreground">用每一分成长，兑换一份喜欢</h2><p className="mt-1 text-xs text-muted-foreground">{mallConfig.notice}</p></div><span className="inline-flex w-fit items-center gap-1.5 rounded-lg bg-primary/[0.07] px-3 py-2 text-xs font-semibold text-primary"><CalendarClock className="size-3.5" aria-hidden="true" />至 {mallConfig.endAt ? new Intl.DateTimeFormat("zh-CN", { month: "2-digit", day: "2-digit" }).format(new Date(mallConfig.endAt)) : "长期"}</span></div><div className="mt-4 flex flex-col gap-3 sm:flex-row"><label className="relative flex min-w-0 flex-1 items-center"><Search className="pointer-events-none absolute left-3 size-4 text-muted-foreground" aria-hidden="true" /><input value={search} onChange={(event) => setSearch(event.target.value)} name="mall-search" autoComplete="off" placeholder="搜索商品名称…" className="h-10 w-full rounded-xl border border-[#d9e4f4] bg-[#fbfdff] pl-9 pr-3 text-sm outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20" /></label><div role="tablist" aria-label="商品分类" className="flex max-w-full gap-1 overflow-x-auto rounded-xl bg-[#f2f6fd] p-1">{categories.map((item) => <button key={item} type="button" role="tab" aria-selected={category === item} onClick={() => setCategory(item)} className={cn("min-h-8 shrink-0 rounded-lg px-3 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/45", category === item ? "bg-white text-primary shadow-sm" : "text-muted-foreground hover:text-foreground")}>{item}</button>)}</div></div></>}>
    <section aria-labelledby="mall-product-list-title"><div className="mb-3 flex items-center justify-between"><h2 id="mall-product-list-title" className="text-base font-bold text-foreground">商品中心</h2><span className="text-xs text-muted-foreground">共 {products.length} 件商品</span></div>{products.length === 0 ? <div className="rounded-2xl border border-dashed border-primary/25 bg-white/70 px-4 py-14 text-center text-sm text-muted-foreground">暂无符合条件的商品</div> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{products.map((product) => { const eligibility = getEligibility(product, child, balance, semesterPoints); return <Link key={product.id} href={`/points-mall/product?${childQuery}&product=${encodeURIComponent(product.id)}`} className="group overflow-hidden rounded-2xl border border-[#dce6f5] bg-white shadow-[0_12px_24px_-22px_rgba(48,78,140,.58)] transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-[0_18px_30px_-22px_rgba(48,78,140,.65)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/45 motion-reduce:transform-none"><div className="aspect-[1.16] overflow-hidden bg-primary/[0.04]"><MallProductImage product={product} className="size-full transition-transform duration-300 group-hover:scale-105 motion-reduce:transform-none" /></div><div className="p-3"><p className="truncate text-sm font-bold text-foreground">{product.name}</p><p className="mt-1 line-clamp-2 min-h-9 text-xs leading-4 text-muted-foreground">{product.description}</p><div className="mt-3 flex items-center justify-between gap-2"><span className="inline-flex items-center gap-1 text-sm font-bold text-brand-orange"><Coins className="size-3.5" aria-hidden="true" />{product.pointsCost}</span><span className={cn("rounded-full px-2.5 py-1 text-[11px] font-semibold", eligibility.available ? "bg-brand-green/12 text-brand-green" : "bg-muted text-muted-foreground")}>{eligibility.available ? "可兑换" : eligibility.reason}</span></div></div></Link> })}</div>}</section>
  </MallFrame>
}
...
export function PointsMallProductPage() {
  ...
  return <MallFrame kind="product" title="商品详情" child={child} balance={balance} cartCount={cartCount}>
...
export function PointsMallCartPage() {
  ...
  const childQuery = `student=${encodeURIComponent(child?.studentId ?? "")}`
  return <MallFrame kind="cart" title="购物车" child={child} balance={balance} cartCount={cartCount}>
...
export function PointsMallCheckoutPage() {
  ...
  return <MallFrame kind="checkout" title="确认兑换" child={child} balance={balance} cartCount={cartCount}>
...
export function PointsMallRecordsPage() {
  ...
  const childQuery = `student=${encodeURIComponent(child?.studentId ?? "")}`
  return <MallFrame kind="records" title="兑换记录" child={child} balance={balance} cartCount={cartCount}>
