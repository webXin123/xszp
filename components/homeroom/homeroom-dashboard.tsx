...
const dashboardActionButtonClass = "inline-flex h-9 w-24 shrink-0 touch-manipulation items-center justify-center gap-1 rounded-lg bg-primary px-2.5 text-xs font-bold text-primary-foreground shadow-[0_7px_16px_-14px_rgba(63,81,188,0.9)] transition-[background-color,box-shadow] duration-200 hover:bg-primary/90 hover:shadow-[0_9px_18px_-14px_rgba(63,81,188,0.95)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/45"
...
export function HomeroomDashboard({ onNavigate }: HomeroomDashboardProps) {
  ...
  const [evaluationDetailPeriod, setEvaluationDetailPeriod] = useState<EvaluationDetailPeriod>("week")
  const [evaluationDetailKind, setEvaluationDetailKind] = useState<EvaluationDetailKind>("all")
  const [evaluationDetailDay, setEvaluationDetailDay] = useState(() => formatDate(new Date()))
  const [evaluationDetailWeek, setEvaluationDetailWeek] = useState(() => getISOWeekKey(new Date()))
  const [evaluationDetailMonth, setEvaluationDetailMonth] = useState(() => formatDate(new Date()).slice(0, 7))
  ...
  const evaluationDetailWeekOptions = useMemo(() => getSemesterWeekKeys(now), [now.getFullYear(), now.getMonth(), now.getDate()])
  const evaluationDetailMonthOptions = useMemo(() => getSemesterMonthKeys(now), [now.getFullYear(), now.getMonth(), now.getDate()])
  const evaluationDetailRecords = useMemo(() => {
    if (!currentClass) return []
    const filtered = evaluationDetailPeriod === "day"
      ? records.filter((record) => record.classId === currentClass.id && record.date === evaluationDetailDay)
      : evaluationDetailPeriod === "week"
        ? getRecordsForWeek(records, currentClass.id, evaluationDetailWeek)
        : records.filter((record) => record.classId === currentClass.id && record.date.startsWith(evaluationDetailMonth))
    return [...filtered].sort((a, b) => `${b.date}-${b.createdAt}`.localeCompare(`${a.date}-${a.createdAt}`))
  }, [currentClass, evaluationDetailDay, evaluationDetailMonth, evaluationDetailPeriod, evaluationDetailWeek, records])
  const evaluationDetailTotals = useMemo(() => evaluationDetailRecords.reduce((totals, record) => {
    if (record.totalDeduction > 0) totals.addition += record.totalDeduction
    if (record.totalDeduction < 0) totals.deduction += Math.abs(record.totalDeduction)
    return totals
  }, { addition: 0, deduction: 0 }), [evaluationDetailRecords])
  const visibleEvaluationDetailRecords = useMemo(() => evaluationDetailKind === "all"
    ? evaluationDetailRecords
    : evaluationDetailRecords.filter((record) => evaluationDetailKind === "addition" ? record.totalDeduction > 0 : record.totalDeduction < 0), [evaluationDetailKind, evaluationDetailRecords])
  const showEvaluationStudentColumn = evaluationDetailRecords.some((record) => record.targetType === "student" || record.studentNames.length > 0)
  ...
  const previousClassWeeklyRank = useMemo(() => {
    ...
    return [...scoringClasses]
      .map((item) => ({ id: item.id, total: computeWeeklyScore(records, item.id, previousWeekKey).total }))
  ...
  const previousClassWeeklyScore = useMemo(
    () => currentClass ? computeWeeklyScore(records, currentClass.id, previousWeekKey).total : 0,
    [currentClass, previousWeekKey, records],
  )

  const previousClassRating = useMemo(() => {
    if (!previousClassWeeklyRank) return undefined
    return findClassRating(classRatingConfigs, previousClassWeeklyRank, previousClassWeeklyScore)
  ...
  return (
    <div className={cn("relative -m-4 flex flex-col gap-5 p-4 sm:-m-6 sm:gap-6 sm:p-6", styles.workspaceHome, styles.homeroomHome)}>
      ...
      <div><p className="text-xs font-semibold tracking-[0.16em] text-primary">班级工作台</p><h1 className="mt-1 text-xl font-bold tracking-tight text-foreground">班主任首页</h1><p data-mobile-secondary="true" className="mt-1 text-xs text-muted-foreground">关注班级表现与本周待办，及时推进评价工作。</p></div>
            ...
            <div className="mt-3 grid gap-2.5 sm:grid-cols-2">
              ...
              <div className="flex min-h-[68px] min-w-0 items-center gap-2.5 rounded-2xl border border-[#f1dfb7] bg-[#fffaf0] px-3 py-2 shadow-[0_10px_22px_-22px_rgba(167,120,33,0.38)]" aria-label="上周流动红旗获得情况">
                <img src={lastWeekFlags[0]?.config?.image ?? "/xszp/images/flag-issued.svg"} alt="" width="38" height="38" fetchPriority="high" className="size-9 shrink-0 object-contain" />
                <span className="min-w-0"><span className="block text-xs font-semibold text-[#a77821]">上周流动红旗</span><span title={lastWeekFlags.map((flag) => flag.config?.name ?? "流动红旗").join("、")} className="block truncate text-sm font-bold text-foreground">{lastWeekFlags.length > 0 ? lastWeekFlags.map((flag) => flag.config?.name ?? "流动红旗").join("、") : "暂未获得"}</span></span>
              </div>
            </div>
          </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <section className="flex h-[320px] min-h-0 flex-col rounded-2xl border border-[#cbd6f7] border-t-2 border-t-brand-orange bg-white p-5 shadow-[0_14px_30px_-26px_rgba(48,62,139,0.62)] sm:p-6" aria-labelledby="week-deductions-title">
          <SectionHeading
            ...
            action={<button type="button" onClick={() => setEvaluationDetailOpen(true)} className={dashboardActionButtonClass}>查看明细<ChevronRight className="size-3.5" aria-hidden="true" /></button>}
          />
        ...
        <DialogContent className="max-h-[88vh] border border-[#cfd8f6] bg-[#f7f9ff] p-0 shadow-[0_24px_70px_-32px_rgba(48,62,139,0.72)] sm:max-w-5xl">
          <DialogHeader className="border-b border-[#dfe5f7] bg-gradient-to-r from-[#eef2ff] via-white to-[#fff8f0] px-5 py-5 pr-12 sm:px-6">
            <div className="flex items-start gap-3">
              <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-[#fff1e9] text-brand-orange"><BarChart3 className="size-5" aria-hidden="true" /></span>
              <div className="min-w-0">
                <DialogTitle className="text-lg font-bold">{currentClass.name} · 班级评价明细</DialogTitle>
                <DialogDescription className="mt-1">按日、周、月查看班级加分与扣分记录及统计数据。</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          <div className="space-y-4 bg-[#f7f9ff] p-5 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#dce4f7] bg-white p-3.5">
              <div className="flex flex-wrap items-center gap-2.5">
                <div className="inline-flex rounded-xl border border-primary/10 bg-[#eef1ff] p-1" role="tablist" aria-label="评价明细周期">
                  {(["day", "week", "month"] as EvaluationDetailPeriod[]).map((item) => <button key={item} type="button" role="tab" aria-selected={evaluationDetailPeriod === item} onClick={() => setEvaluationDetailPeriod(item)} className={cn("min-h-10 min-w-16 touch-manipulation rounded-lg px-3 text-xs font-bold transition-[background-color,color,box-shadow] duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50", evaluationDetailPeriod === item ? "bg-gradient-to-r from-primary to-primary-2 text-primary-foreground shadow-[0_7px_16px_-9px_rgba(63,81,188,0.92)]" : "text-[#687898] hover:bg-white hover:text-primary")}>{item === "day" ? "按天" : item === "week" ? "按周" : "按月"}</button>)}
                </div>
                {evaluationDetailPeriod === "day" && <label className="flex h-10 items-center gap-2 rounded-xl border border-[#dbe3f7] bg-[#fbfcff] px-3"><CalendarDays className="size-3.5 text-primary" aria-hidden="true" /><span className="sr-only">选择评价明细日期</span><Input type="date" name="homeroom-evaluation-detail-day" autoComplete="off" aria-label="选择评价明细日期" max={today} value={evaluationDetailDay} onChange={(event) => setEvaluationDetailDay(event.target.value)} className="h-8 w-[132px] border-0 bg-transparent p-0 text-xs font-semibold shadow-none focus-visible:ring-0" /></label>}
                {evaluationDetailPeriod === "week" && <label className="flex h-10 items-center gap-2 rounded-xl border border-[#dbe3f7] bg-[#fbfcff] px-3"><CalendarDays className="size-3.5 text-primary" aria-hidden="true" /><span className="sr-only">选择评价明细周次</span><select aria-label="选择评价明细周次" value={evaluationDetailWeek} onChange={(event) => setEvaluationDetailWeek(event.target.value)} className="h-8 min-w-40 bg-transparent text-xs font-semibold text-foreground outline-none"><option value="" disabled>选择周次</option>{evaluationDetailWeekOptions.map((item) => <option key={item} value={item}>{item === weekKey ? `本周 · ${formatWeekLabel(item)}` : formatWeekLabel(item)} · {formatDateRangeLabel(item)}</option>)}</select></label>}
                {evaluationDetailPeriod === "month" && <label className="flex h-10 items-center gap-2 rounded-xl border border-[#dbe3f7] bg-[#fbfcff] px-3"><CalendarDays className="size-3.5 text-primary" aria-hidden="true" /><span className="sr-only">选择评价明细月份</span><select aria-label="选择评价明细月份" value={evaluationDetailMonth} onChange={(event) => setEvaluationDetailMonth(event.target.value)} className="h-8 min-w-32 bg-transparent text-xs font-semibold text-foreground outline-none"><option value="" disabled>选择月份</option>{evaluationDetailMonthOptions.map((item) => <option key={item} value={item}>{item === today.slice(0, 7) ? `本月 · ${formatMonthLabel(item)}` : formatMonthLabel(item)}</option>)}</select></label>}
              </div>
              <span className="text-xs font-medium text-muted-foreground">{evaluationDetailPeriod === "day" ? evaluationDetailDay : evaluationDetailPeriod === "week" ? `${formatWeekLabel(evaluationDetailWeek)} · ${formatDateRangeLabel(evaluationDetailWeek)}` : formatMonthLabel(evaluationDetailMonth)}</span>
            </div>

            <div className="grid grid-cols-2 gap-2.5 sm:gap-3" aria-live="polite">
              <div className="rounded-2xl border border-[#c6e9d4] bg-[#f3fcf6] px-3 py-2.5 sm:p-3"><p className="text-xs font-semibold text-[#43835b]">加分合计</p><p className="mt-0.5 text-[18px] leading-6 font-bold tabular-nums text-emerald-700">+{evaluationDetailTotals.addition.toFixed(1)} <span className="text-xs font-semibold">分</span></p><p className="mt-0.5 text-xs text-[#6b9279]">{evaluationDetailRecords.filter((record) => record.totalDeduction > 0).length} 条加分记录</p></div>
              <div className="rounded-2xl border border-[#f2d0c3] bg-[#fff7f3] px-3 py-2.5 sm:p-3"><p className="text-xs font-semibold text-[#a8644b]">扣分合计</p><p className="mt-0.5 text-[18px] leading-6 font-bold tabular-nums text-rose-700">-{evaluationDetailTotals.deduction.toFixed(1)} <span className="text-xs font-semibold">分</span></p><p className="mt-0.5 text-xs text-[#a47c70]">{evaluationDetailRecords.filter((record) => record.totalDeduction < 0).length} 条扣分记录</p></div>
            </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-[#dce4f7] bg-white">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#e5eaf7] bg-[#fbfcff] px-3.5 py-3 sm:px-4">
                <div><h3 className="text-sm font-bold text-foreground">评价记录</h3><p className="mt-0.5 text-xs text-muted-foreground">当前显示 {visibleEvaluationDetailRecords.length} 条</p></div>
                <div className="inline-flex rounded-lg border border-[#dbe3f7] bg-white p-1" role="tablist" aria-label="评价记录类型">
                  {(["all", "addition", "deduction"] as EvaluationDetailKind[]).map((item) => <button key={item} type="button" role="tab" aria-selected={evaluationDetailKind === item} onClick={() => setEvaluationDetailKind(item)} className={cn("min-h-8 touch-manipulation rounded-md px-2.5 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50", evaluationDetailKind === item ? item === "deduction" ? "bg-[#fff1e9] text-brand-orange" : item === "addition" ? "bg-[#eaf8f1] text-brand-green" : "bg-[#eef2ff] text-primary" : "text-muted-foreground hover:bg-[#f7f9ff] hover:text-foreground")}>{item === "all" ? "全部" : item === "addition" ? "只看加分" : "只看扣分"}</button>)}
                </div>
              </div>
              {visibleEvaluationDetailRecords.length === 0 ? <div className="px-4 py-12 text-center"><p className="text-sm font-semibold text-foreground">当前范围暂无{evaluationDetailKind === "addition" ? "加分" : evaluationDetailKind === "deduction" ? "扣分" : "评价"}记录</p><p className="mt-1 text-xs text-muted-foreground">可切换日期或周期查看其他班级评价数据。</p></div> : <div className="overflow-x-auto"><table className={cn("w-full border-collapse text-left", showEvaluationStudentColumn ? "min-w-[820px]" : "min-w-[720px]")}><caption className="sr-only">{currentClass.name} {evaluationDetailPeriod === "day" ? evaluationDetailDay : evaluationDetailPeriod === "week" ? formatWeekLabel(evaluationDetailWeek) : formatMonthLabel(evaluationDetailMonth)}班级评价明细</caption><thead><tr className="border-b border-[#e5eaf7] bg-[#f7f9ff] text-xs font-bold text-muted-foreground"><th scope="col" className="w-28 px-4 py-3">日期</th><th scope="col" className="px-4 py-3">评价项目 / 说明</th>{showEvaluationStudentColumn && <th scope="col" className="w-32 px-4 py-3">扣分学生</th>}<th scope="col" className="w-24 px-4 py-3">操作人</th><th scope="col" className="w-28 px-4 py-3 text-right">分值</th></tr></thead><tbody>{visibleEvaluationDetailRecords.map((record) => { const addition = record.totalDeduction > 0; const meta = [record.note, record.studentNames.length > 0 ? `涉及学生：${record.studentNames.join("、")}` : ""].filter(Boolean).join(" · ") || "未填写说明"; return <tr key={record.id} className="border-b border-[#edf0fa] align-top transition-colors last:border-0 hover:bg-[#fbfcff]"><td className="px-4 py-3 text-xs tabular-nums text-muted-foreground"><time dateTime={record.date}>{record.date}</time></td><td className="px-4 py-3"><p className="text-sm font-semibold text-foreground">{record.level1} <span className="font-normal text-muted-foreground">· {record.level2}</span></p><p className="mt-1 break-words text-xs leading-5 text-muted-foreground">{meta}</p></td>{showEvaluationStudentColumn && <td className="px-4 py-3 text-xs font-medium text-foreground">{record.studentNames.length > 0 ? record.studentNames.join("、") : "—"}</td>}<td className="px-4 py-3 text-xs text-muted-foreground">{record.operatorName || "系统"}</td><td className="px-4 py-3 text-right"><span className={cn("inline-flex items-center justify-end gap-1 text-sm font-bold tabular-nums", addition ? "text-emerald-700" : "text-rose-700")}>{addition ? <PlusCircle className="size-3.5" aria-hidden="true" /> : <MinusCircle className="size-3.5" aria-hidden="true" />}{addition ? "+" : "−"}{Math.abs(record.totalDeduction).toFixed(1)}<span className="text-xs font-semibold">分</span></span><span className="mt-1 block text-xs text-muted-foreground">{addition ? "加分" : "扣分"}</span></td></tr> })}</tbody></table></div>}
            </div>
          </div>
        </DialogContent>
      </Dialog>
