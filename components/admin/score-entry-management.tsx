...
function PeScoreEntryManagement() {
  ...
  return <div className="flex flex-col gap-4">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="flex items-start gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-primary text-primary-foreground shadow-[0_9px_18px_-11px_rgba(63,81,188,0.82)]"><HeartPulse className="size-5" aria-hidden="true" /></span><div><h3 className="text-base font-bold text-foreground">体质健康录入</h3><p className="mt-0.5 text-xs text-muted-foreground">{getSemesterLabel()} · 按班级、性别统览体质健康成绩录入进度。</p></div></div>
    ...
    <Dialog open={drawerOpen} onOpenChange={setDrawerOpen}>
      <DialogContent className="fixed left-auto right-0 top-0 grid h-full max-h-full w-full max-w-md translate-x-0 translate-y-0 gap-0 overflow-hidden rounded-none rounded-l-2xl border-[#cbd7f7] bg-white p-0 shadow-[-18px_0_44px_-28px_rgba(53,67,150,0.5)] sm:max-w-md">
        <DialogHeader className="border-b border-[#dce4fa] border-t-4 border-t-primary bg-[#f7f9ff] p-5 pr-12"><DialogTitle className="flex items-center gap-2 text-lg"><span className="flex size-8 items-center justify-center rounded-xl bg-primary text-primary-foreground"><HeartPulse className="size-4" aria-hidden="true" /></span>{task ? "调整体质健康录入任务" : "发布体质健康录入任务"}</DialogTitle><DialogDescription>设置录入班级和时间范围，发布后可持续查看进度。</DialogDescription></DialogHeader>
