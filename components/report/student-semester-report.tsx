...
export function StudentSemesterReport({ semesterLabel, student, className, gradeName, homeroomTeacher, semesterPoints, totalPoints, fiveEducation, academicScores, fitnessMetrics, honors, activities }: StudentSemesterReportProps) {
  ...
  return <article className="report-sheet relative mx-auto w-full max-w-[941px] overflow-hidden bg-[#effaff] font-serif text-[#174d91] shadow-[0_24px_70px_-34px_rgba(38,105,161,.55)]">
    <div className="relative z-10 px-3 pb-4 pt-0 sm:px-[4.46%] sm:pb-6">
      ...
      <Section icon={BookOpenText} title="各科期末总评成绩" aside="求真笃学　知行合一" className="mt-3"><div className="overflow-x-auto px-3 pb-4 pt-3 sm:px-4"><table className="w-full min-w-[280px] overflow-hidden rounded-xl border border-[#9cc9ea] text-xs sm:text-[16px]"><caption className="sr-only">{student.name}本学期各科总评成绩</caption><thead className="bg-[#b9dcf4] text-[#15529b]"><tr><th className="px-2 py-1.5 text-center font-bold sm:px-3">科目</th><th className="px-2 py-1.5 text-center font-bold sm:px-3">总评</th><th className="px-2 py-1.5 text-center font-bold sm:px-3">表现</th></tr></thead><tbody>{academicScores.map((score, index) => <tr key={score.subject} className={cn("border-t border-[#c1dff4]", index % 2 === 1 && "bg-[#eef8ff]")}><th className="px-2 py-1 text-center font-semibold sm:px-3">{score.subject}</th><td className="px-2 py-1 text-center font-semibold sm:px-3">{score.score}</td><td className="px-2 py-1 text-center font-semibold sm:px-3">{score.level}</td></tr>)}</tbody></table></div></Section>
